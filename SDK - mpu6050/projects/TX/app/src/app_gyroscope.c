 ...
 */
void acc_gyro_sample_data(int16_t *gyro_raw, int16_t *acc_raw)
{
    ...
    imu_9.i_acc[0] = (acc_raw_8[0] << 8) + acc_raw_8[1];
    imu_9.i_acc[1] = (acc_raw_8[2] << 8) + acc_raw_8[3];
    imu_9.i_acc[2] = (acc_raw_8[4] << 8) + acc_raw_8[5];
    
    imu_9.i_gyro[0] = (gyro_raw_8[0] << 8) + gyro_raw_8[1];
    imu_9.i_gyro[1] = (gyro_raw_8[2] << 8) + gyro_raw_8[3];
    imu_9.i_gyro[2] = (gyro_raw_8[4] << 8) + gyro_raw_8[5];
...
{ 
	imu_9.f_acc[0] = (float)(ax) / ACCEL_SSF;  // 加速度量程为:±2G      获取到的加速度计数据 除以 16393 ，可以转化为带物理单位的数据，单位：g(m/s^2)
	imu_9.f_acc[1] = (float)(ay) / ACCEL_SSF;
	imu_9.f_acc[2] = (float)(az) / ACCEL_SSF;

	imu_9.f_gyro[0] = (float)(gx)/GYRO_SSF; //  陀螺仪量程为:±500dps  获取到的陀螺仪数据除以 57.1，    可以转化为带物理单位的数据，单位为：°/s
	imu_9.f_gyro[1] = (float)(gy)/GYRO_SSF;
	imu_9.f_gyro[2] = (float)(gz)/GYRO_SSF;
}
...
{
	if(abs(imu_9.i_gyro[0])>3000 && abs(imu_9.i_gyro[2])<1500){             //上下
//		ey += imu_9.i_gyro[2]* 0.001;
//		if(ey > 1000)ey = 0;
		imu_9.i_gyro[2] = 0;
	}else if(abs(imu_9.i_gyro[2]) > 3000 && abs(imu_9.i_gyro[0])<1500){           //左右
//		ex += imu_9.i_gyro[0]* 0.001;
//		if(ex > 1000)ex = 0;
		imu_9.i_gyro[0] = 0;
	}	       
...
void calibrate_offset()
{
    ...
    for (int i = 0; i < num_samples; i++)
    {
				...
				imu_9.acc_zero[0] = ax_offset_sum/num_samples;
				imu_9.acc_zero[1] = ay_offset_sum/num_samples;
				imu_9.acc_zero[2] = az_offset_sum/num_samples;
				imu_9.gyro_zero[0] = gx_offset_sum/num_samples;
				imu_9.gyro_zero[1] = gy_offset_sum/num_samples;
				imu_9.gyro_zero[2] = gz_offset_sum/num_samples;
}
...
void imu_final_data_get(void)
{
	...
	int16_t filter_gx ,filter_gy,filter_gz ;
	  ...
	  acc_gyro_sample_data(imu_9.i_gyro,imu_9.i_acc); //采样得到九轴原始数据
    Filter_threshold();
...
//	numbers[3] = imu_9.f_gyro[0];
//	numbers[4] = imu_9.f_gyro[1];
//	numbers[5] = imu_9.f_gyro[2];
...
void IMUupdate(float gx, float gy, float gz, float ax, float ay, float az) // g表陀螺仪，a表加计
{
    ...
    if (norm == 0) return;                    // 加计处于自由落体状态时不进行姿态解算，因为会产生分母无穷大的情况
    ...
    yaw = atan2(2 * q1 * q2 + 2 * q0 * q3, 1 - 2 * q2 * q2 - 2 * q3 * q3 ) * 57.3;  // unit:degree
    pitch = asin(-2 * q1 * q3 + 2 * q0 * q2) * 57.3;                                // unit:degree
    roll = atan2(2 * q2 * q3 + 2 * q0 * q1, q0 * q0- q1 * q1 - q2 * q2  + q3*q3) * 57.3; // 57.3 = 180/PI
		numbers[0] = (int16)roll;
//    numbers[1] = pitch;
//    numbers[2] = yaw;
...
//    Filter_threshold((int16_t * )imu_9.f_gyro, gyro_filter);
    //uart_printf("gyro_raw: 	X=%d, Y=%d, Z=%d \r\n", gyro_raw[0], gyro_raw[1], gyro_raw[2]);
    //uart_printf("gyro_filter: 	X=%d, Y=%d, Z=%d \r\n", imu_9.f_gyro[0], imu_9.f_gyro[1], imu_9.f_gyro[2]);

    
		//IMUupdate(imu_9.f_gyro[0]*0.0175, imu_9.f_gyro[1]*0.0175, imu_9.f_gyro[2]*0.0175, imu_9.f_acc[0], imu_9.f_acc[1], imu_9.f_acc[2]);
    ...
    float acc_angle_x = calculate_acc_angle(acc_raw, gyro_raw);
		float angle_p = yijiehubu_P(acc_angle_x, imu_9.f_gyro[2]);
