...
void app_timer0_1_int_cb(void)
{
    ...
    if(mouse_en && mouse_flag == 0 && system_data.system_mode == SYSTEM_NORMAL){
        if(++mouse_ct >= 8){
            mouse_ct = 0;
            mouse_flag = 1;
        }
    ...
    if(!mouse_en && !status_report_flag){
        if(++status_report_ct >= 10000){
            status_report_flag = 1;
            status_report_ct = 0;
        }
    }

    if(!mouse_en && !bat_report_flag){
        if(++bat_report_ct >= 10000){
            bat_report_flag = 0;
            bat_report_ct = 0;
        }
    }
