...
void user_application_normal_mode(void)
{
    ...
    while (system_data.system_mode == SYSTEM_NORMAL)
    {
        ...
        if(disconnect_flag){
            disconnect_flag = 0;
            disconnect_ct = 0;
            status_report(0x00, 0x00, 0, 0x09);
        }
    }
}
...
void user_rf_rx_handler()
{
    ...
    case KeyFuncKeyBottomLongPress:
        ...
        break;
    case 0x0A: // 鼠标数据发送
        if(uRXDataLens != 6) break;
        mouse_send(rf_fifo_data[5],((rf_fifo_data[1] << 8) + rf_fifo_data[2]),((rf_fifo_data[3] << 8) + rf_fifo_data[4]));
        // mouse_send(rf_fifo_data[1],rf_fifo_data[2]);
    ...
    case 0x0E:
        ...
        key_send(0, KEY_P);
        key_send(0, KEY_S);
        key_send(KEY_MOD_LSHIFT, KEY_SEMICOLON);
        key_send(0, KEY_SLASH);
        key_send(0, 0);
        key_send(0, KEY_SLASH);
        key_send(0, KEY_W);
        key_send(0, 0);
...
// action: absolute 0, relative 1;
void mouse_send(uint8_t action,int16_t x, int16_t y)
{
    uint8_t arr[6];
    // abs
    if(action == 0){
        arr[0] = 0x01; // 相对/绝对位置移动方式 HID REPORT ID
        arr[1] = 0x00;
    ...
    // releative
    if(action == 1){
        arr[0] = 0x02; // 相对/绝对位置移动方式 HID REPORT ID
        arr[1] = 0x00;
        arr[2] = x & 0x00FF; // 鼠标模式 x轴
        arr[3] = y & 0x00FF; // 鼠标模式 y轴
        arr[4] = 0x00;
        AplUsb_StartTx(1, arr, 5);
    }
}
