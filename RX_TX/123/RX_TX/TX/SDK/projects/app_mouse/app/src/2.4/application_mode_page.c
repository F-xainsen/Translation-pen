...
uint32_t rf_address_tx_page[RF_ADDRESS_LEN] = {0x20, 0x34, 0x56, 0x42, 0x46};	// {0x20, 0x34, 0x56, 0x00, 0x00};
uint32_t rf_address_tx_page1[RF_ADDRESS_LEN] = {0x15, 0x59, 0x23, 0xC6, 0x29};	// {0x20, 0x34, 0x56, 0x00, 0x00};
...
//    uint8_t temp[14];
	...
	uart_printf("rf_fifo_data[0]=%x,rf_fifo_data[1]=%x,rf_fifo_data[2]=%x,\
							rf_fifo_data[3]=%x,rf_fifo_data[4]=%x\r\n",\
							rf_fifo_data[0],rf_fifo_data[1],rf_fifo_data[2],rf_fifo_data[3],rf_fifo_data[4]);
	FLUSH_RX;
...
//				{
//						// 添加发送失败的调试信息
//						uart_printf("Send failed: status=0x%x\r\n", TRX_IRQ_STATUS);
//						uart_printf("After send: RF_flag=0x%x\r\n", RF_flag);
//						uart_printf("TX_FIFO_STATUS=0x%x\r\n", TRX_FIFO_STATUS);
//				}
